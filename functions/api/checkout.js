...
export async function onRequest(context) {
  ...
  try {
    // Get Stripe API key from environment variable (using STRIPE_API_KEY_CART as set in Cloudflare Pages)
    ...
    for (const item of cart) {
      ...
      if (!priceId) {
        return new Response(
          JSON.stringify({ error: `Product ID ${item.id} not found in price mapping` }),
          {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          }
